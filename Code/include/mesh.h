...
public:
    ...
    inline Triangle & operator= (const Triangle & t2) {
     ...
     inline  Vec3Df centroid(Vec3Df A, Vec3Df B, Vec3Df C) {
         float xAverage = (A[0] + B[0] + C[0])/3;
         float yAverage = (A[1] + B[1] + C[1])/3;
         float zAverage = (A[2] + B[2] + C[2])/3;
         return Vec3Df(xAverage,yAverage,zAverage);
     }
	//vertex position
    unsigned int v[3];
	//texture coordinate
